...
function Menu:InitVars()
	...
	{
		{"Hızlı bağlantı", Menu.QuickConnect},
		{"Sunucu Tarayıcısı", Menu.ServerBrowser},
		{"Başarılar", Menu.Achievements},
		{"Ayarlar", Menu.Settings},
		{"çıkmak", Menu.Quit}
	}
	-- Pause Menu Buttons
	...
	{
		{"özgeçmiş", Menu.Resume},
		{"Sık Kullanılanlara Ekle", Menu.AddToFavorites},
		{"kesmek", Menu.Disconnect},
		{"Hızlı Bağlantı", Menu.QuickConnect},
		{"Sunucu Tarayıcısı", Menu.ServerBrowser},
		{"Başarılar", Menu.Achievements},
		{"Ayarlar", Menu.Settings},
	...
	self.colors["DBlue"] = Color(0, 5, 255)	
...
function Menu:RenderInfo()
	...
	Render:DrawText(Vector2.Zero + Vector2(self.sidebarPadding, self.sidebarPadding + self.infoNameFullHeight), "Online Oyuncular: ", self.colors["Blue"], self.infoCountFontSize)
	local width = Render:GetTextWidth("Online Oyuncular: ", self.infoCountFontSize)
...
function Menu:RenderFeatured()
	...
	Render:SetFont(AssetLocation.Disk, self.fonts["Text"])
	Render:DrawText(lineOffset, "Hepsini gör", self.colors["Blue"], self.featuredViewAllFontSize)
	self:AddHitbox(lineOffset, Render:GetTextSize("Hepsini gör", self.featuredViewAllFontSize), Menu.ServerBrowser, ServerBrowserPage.Featured)
...
function Menu:RenderFavorites()
	...
	Render:SetFont(AssetLocation.Disk, self.fonts["Text"])
	Render:DrawText(self.contentOffset, "Hepsini gör", self.colors["Orange"], self.favoriteViewAllFontSize)
	self:AddHitbox(self.contentOffset, Render:GetTextSize("Hepsini gör", self.favoriteViewAllFontSize), Menu.ServerBrowser, ServerBrowserPage.Favorites)
